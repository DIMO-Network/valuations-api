...
install: $(APPS)
	@mkdir -p bin
	@cp $(PATHINSTBIN)/valuations-api ./bin/
...
docker: deps
	@docker build -f ./resources/docker/Dockerfile . -t dimozone/valuations-api:$(VER_CUT)
	@docker tag dimozone/valuations-api:$(VER_CUT) dimozone/valuations-api:latest
...
test: $(APPS)
	...
	@$(PATHINSTBIN)/valuations-api test ./config/test/...
