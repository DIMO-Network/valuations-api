...
var ValuationWhere = struct {
	...
	DefinitionID:          whereHelpernull_String{field: "\"valuations_api\".\"valuations\".\"definition_id\""},
...
var (
	valuationAllColumns            = []string{"id", "device_definition_id", "vin", "offer_metadata", "edmunds_metadata", "created_at", "updated_at", "drivly_pricing_metadata", "request_metadata", "vincario_metadata", "definition_id", "token_id"}
	valuationColumnsWithoutDefault = []string{"id", "vin"}
	valuationColumnsWithDefault    = []string{"device_definition_id", "offer_metadata", "edmunds_metadata", "created_at", "updated_at", "drivly_pricing_metadata", "request_metadata", "vincario_metadata", "definition_id", "token_id"}
...
func (o *Valuation) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(valuationAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(valuationPrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert valuations, could not build conflict column list")
			}

			conflict = make([]string, len(valuationPrimaryKeyColumns))
			copy(conflict, valuationPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"valuations_api\".\"valuations\"", updateOnConflict, ret, update, conflict, insert, opts...)
...
func (o *Valuation) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	return ValuationExists(ctx, exec, o.ID)
}
